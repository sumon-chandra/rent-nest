...
rentalReqRouter.get("/my-properties", auth(Role.TENANT), rentalRequestController.getMyProperties);
rentalReqRouter.get("/:id", auth(Role.ADMIN, Role.TENANT, Role.LANDLORD), rentalRequestController.getRentalRequestById);
rentalReqRouter.patch("/update-status/:id", auth(Role.LANDLORD, Role.ADMIN), rentalRequestController.updateRentalRequest);
